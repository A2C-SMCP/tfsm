...
class TestNestedTransitions(TestTransitions):
    ...
    def test_init_machine_with_hella_arguments(self):
        states = [self.state_cls("State1"), "State2", {"name": "State3", "on_enter": "hello_world"}]
        transitions = [{"trigger": "advance", "source": "State2", "dest": "State3"}]
        s = Stuff()
        self.stuff.machine_cls(model=s, states=states, transitions=transitions, initial="State2")
        s.advance()
    ...
    def test_init_machine_with_nested_states(self):
        ...
        m.to(f"B{State.separator}1")
        self.assertEqual(m.state, f"B{State.separator}1")
    ...
    def test_nested_definitions(self):
        ...
        state = {"name": "B", "children": ["1", "2"], "tfsm": [["jo", "1", "2"]], "initial": "2"}
        m = self.stuff.machine_cls(
            initial="A", states=["A", state], transitions=[["go", "A", "B"], ["go", f"B{separator}2", f"B{separator}1"]]
        )
        self.assertTrue(m.is_A())
        m.go()
        self.assertEqual(m.state, f"B{separator}2")
        m.go()
        self.assertEqual(m.state, f"B{separator}1")
        m.jo()
    ...
    def test_conditions(self):
        ...
        s.machine.add_transition("advance", "C", "D", unless=["this_fails", "this_passes"])
        s.advance()
        self.assertEqual(s.state, "B")
        s.advance()
        self.assertEqual(s.state, "C")
    ...
    def test_add_custom_state(self):
        ...
        s.machine.add_transition("run", "E", f"C{State.separator}3{State.separator}a")
        s.go()
        self.assertEqual(f"E{State.separator}1", s.state)
        s.walk()
        self.assertEqual(f"E{State.separator}3", s.state)
        s.run()
        self.assertEqual(f"C{State.separator}3{State.separator}a", s.state)

    def test_add_nested_state(self):
        m = self.machine_cls(states=["A"], initial="A")
        m.add_state(f"B{self.state_cls.separator}1{self.state_cls.separator}a")
    ...
    def test_enter_exit_nested_state(self):
        ...
        transitions = [["go", "A", f"C{State.separator}1"], ["go", "C", "D"]]
        m = self.stuff.machine_cls(states=states, transitions=transitions, initial="A")
        m.go()
        ...
        m.to(f"C{State.separator}1")
        self.assertEqual(8, mock.call_count)
        m.to(f"C{State.separator}2")
        self.assertEqual(9, mock.call_count)

    def test_ordered_transitions(self):
        State = self.state_cls
        states = [
            {"name": "first", "children": ["second", "third", {"name": "fourth", "children": ["fifth", "sixth"]}, "seventh"]},
            "eighth",
            "ninth",
        ]
        ...
        m.next_state()
        m.next_state()
        self.assertEqual(f"first{State.separator}third", m.state)
        m.next_state()
        m.next_state()
        self.assertEqual(f"first{State.separator}fourth{State.separator}fifth", m.state)
        m.next_state()
        m.next_state()
        self.assertEqual(f"first{State.separator}seventh", m.state)
        m.next_state()
        m.next_state()
        ...
        m.next_state()
        self.assertEqual(m.state, f"first{State.separator}second")
    ...
    def test_pickle(self):
        ...
        states = ["A", "B", {"name": "C", "children": ["1", "2", {"name": "3", "children": ["a", "b", "c"]}]}, "D", "E", "F"]
        transitions = [
            {"trigger": "walk", "source": "A", "dest": "B"},
            {"trigger": "run", "source": "B", "dest": "C"},
            {"trigger": "sprint", "source": "C", "dest": "D"},
    ...
    def test_callbacks_duplicate(self):
        ...
        transitions = [
            {"trigger": "walk", "source": "A", "dest": "C", "before": "before_change", "after": "after_change"},
            {"trigger": "run", "source": "B", "dest": "C"},
        ]
    ...
    def test_example_one(self):
        ...
        machine = self.stuff.machine_cls(
            states=states, transitions=transitions, initial="standing", ignore_invalid_triggers=True, name="Machine 1"
        )
    ...
    def test_intial_state(self):
        separator = self.state_cls.separator
        states = [
            {"name": "A", "children": ["1", "2"], "initial": "2"},
            {"name": "B", "initial": "2", "children": ["1", {"name": "2", "initial": "a", "children": ["a", "b"]}]},
        ]
        transitions = [["do", "A", "B"], ["do", f"B{separator}2", f"B{separator}1"]]
        m = self.stuff.machine_cls(states=states, transitions=transitions, initial="A")
        self.assertEqual(m.state, f"A{separator}2")
        m.do()
        self.assertEqual(m.state, f"B{separator}2{separator}a")
        self.assertTrue(m.is_B(allow_substates=True))
        m.do()
        self.assertEqual(m.state, f"B{separator}1")
        m = self.stuff.machine_cls(states=states, transitions=transitions, initial=f"B{separator}2{separator}b")
        self.assertTrue(f"B{separator}2{separator}b", m.state)
    ...
    def test_get_triggers(self):
        ...
        transitions = [
            ...
            ["go", "standing", "walking"],
            ["stop", "walking", "standing"],
            {"trigger": "drink", "source": "*", "dest": f"caffeinated{seperator}dithering", "conditions": "is_hot", "unless": "is_too_hot"},
            ["walk", f"caffeinated{seperator}dithering", f"caffeinated{seperator}running"],
            ["relax", "caffeinated", "standing"],
        ]

        machine = self.stuff.machine_cls(states=states, transitions=transitions, auto_transitions=False)
        trans = machine.get_triggers(f"caffeinated{seperator}dithering")
    ...
    def test_get_nested_transitions(self):
        ...
        states = [
            ...
            {
                "name": "B",
                "children": ["1", "2", {"name": "3", "children": ["a", "b"], "tfsm": [["inner", "a", "b"], ["inner", "b", "a"]]}],
                "tfsm": [["mid", "1", "1"], ["mid", "2", "3"], ["mid", "3", "1"], ["mid2", "2", "3"], ["mid_loop", "1", "1"]],
            },
        ]
        transitions = [["outer", "A", "B"], ["outer", ["A", "B"], "C"]]
        ...
        self.assertEqual(3, len(machine.get_transitions(dest=f"B{seperator}1")))
        self.assertEqual(2, len(machine.get_transitions(source=f"B{seperator}2", dest=f"B{seperator}3")))
        self.assertEqual(1, len(machine.get_transitions(source=f"B{seperator}3{seperator}a", dest=f"B{seperator}3{seperator}b")))
        ...
        self.assertEqual(3, len(machine.get_transitions(source=machine.states["B"].states["3"].states["b"], delegate=True)))
    ...
    def test_trigger_parent(self):
        ...
        machine = self.machine_cls(
            model, states=[{"name": "A", "children": ["1", "2"]}], transitions=[["go", "A", "A_2"], ["enter", "A", "A_1"]], initial="A"
        )
    ...
    def test_trigger_parent_model_self(self):
        ...
        machine = CustomMachine(
            states=[{"name": "A", "children": ["1", "2"]}], transitions=[["go", "A", "A_2"], ["enter", "A", "A_1"]], initial="A"
        )
        machine.enter()
            ...
            states=[
                {
                    ...
                    "children": ["1", "2"],
                    "initial": "1",
                    "tfsm": [{"trigger": "go", "source": "1", "dest": "2", "conditions": self.stuff.this_fails}],
                },
                "B",
            ],
            transitions=[["go", "B", "A"]],
    ...
    def test_exception_in_state_enter_exit(self):
        ...
        states = ["A", {"name": "B", "initial": "1", "children": ["1", "2"]}, {"name": "C", "initial": "1", "children": ["1", "2"]}]
        model = Model()
    ...
    def test_queued_callbacks(self):
        states = ["initial", {"name": "A", "children": [{"name": "1", "on_enter": "go"}, "2"], "tfsm": [["go", "1", "2"]], "initial": "1"}]
        machine = self.machine_cls(states=states, initial="initial", queued=True)
        machine.to_A()
        self.assertEqual(f"A{self.state_cls.separator}2", machine.state)
        ...
        states = [
            {
                ...
                "states": [
                    {
                        "name": "B",
                        "states": [{"name": "C", "states": ["1", "2"], "initial": "1"}],
                        "tfsm": [["go", "C_1", "C_2"]],
                        "initial": "C",
                    }
                ],
    ...
    def test_auto_transitions_from_nested_callback(self):
        ...
        states = [
            {"name": "b", "children": [{"name": "c", "on_exit": fail, "on_enter": "to_b_c_ca", "children": ["ca", "cb"]}, "d"]},
        ]
    ...
    def test_get_nested_triggers(self):
        transitions = [
            ...
            ["goC", "B", "C"],
            ["goA", "*", "A"],
            ["goF1", [f"C{self.machine_cls.separator}1", f"C{self.machine_cls.separator}2"], "F"],
            ["goF2", "C", "F"],
    ...
    def test_wildcard_src_reflexive_dest(self):
        ...
        machine.reflexive()
        self.assertTrue(machine.is_A())
        state_name = f"C{self.state_cls.separator}2"
...
class TestSeparatorsBase(TestCase):
    ...
    def test_add_nested_state(self):
        m = self.machine_cls(states=["A"], initial="A")
        m.add_state(f"B{self.state_cls.separator}1{self.state_cls.separator}a")
        m.add_state(f"B{self.state_cls.separator}2{self.state_cls.separator}b")
        self.assertIn("B", m.states)
        self.assertIn("1", m.states["B"].states)
        self.assertIn("a", m.states["B"].states["1"].states)
    ...
    def test_enter_exit_nested(self):
        separator = self.state_cls.separator
        s = self.stuff
        s.machine.add_transition("advance", "A", f"C{separator}3")
        s.machine.add_transition("reverse", "C", "A")
        s.machine.add_transition("lower", [f"C{separator}1", f"C{separator}3"], f"C{separator}3{separator}a")
        s.machine.add_transition("rise", f"C{separator}3", f"C{separator}1")
        s.machine.add_transition("fast", "A", f"C{separator}3{separator}a")
        ...
        s.advance()
        self.assertEqual(f"C{separator}3", s.state)
        self.assertEqual(2, s.level)
        self.assertEqual(3, s.transitions)  # exit A; enter C,3
        s.lower()
        self.assertEqual(s.state, f"C{separator}3{separator}a")
    ...
    def test_state_change_listeners(self):
        ...
        s.machine.add_transition("reverse", "C", "A")
        s.machine.add_transition("lower", "C%s1" % State.separator, f"C{State.separator}3{State.separator}a")
        s.machine.add_transition("rise", "C%s3" % State.separator, "C%s1" % State.separator)
        s.machine.add_transition("fast", "A", f"C{State.separator}3{State.separator}a")
        s.machine.on_enter_C("hello_world")
        s.machine.on_exit_C("goodbye")
        s.machine.on_enter(f"C{State.separator}3{State.separator}a", "greet")
        s.machine.on_exit("C%s3" % State.separator, "meet")
        s.advance()
        self.assertEqual(s.state, "C%s1" % State.separator)
        self.assertEqual(s.message, "Hello World!")
        s.lower()
        self.assertEqual(s.state, f"C{State.separator}3{State.separator}a")
    ...
    def test_ordered_with_graph(self):
        ...
        states = ["A", "B", {"name": "C", "children": ["1", "2", {"name": "3", "children": ["a", "b", "c"]}]}, "D", "E", "F"]
        machine = CustomHierarchicalGraphMachine(
            states=states, initial="A", auto_transitions=False, ignore_invalid_triggers=True, graph_engine="graphviz"
        )
    ...
    def test_example_two(self):
        ...
        self.assertEqual(machine.state, f"C{separator}3{separator}a")
        machine.to(f"C{separator}2")  # exit C↦3↦a, exit C↦3, enter C↦2
        self.assertEqual(machine.state, f"C{separator}2")
    ...
    def test_machine_may_transitions(self):
        ...
        transitions = [
            ...
            {"trigger": "run_fast", "source": "C", "dest": f"C{self.separator}1"},
            {"trigger": "sprint", "source": "C", "dest": "D"},
        ]
        m = self.stuff.machine_cls(states=states, transitions=transitions, initial="A", auto_transitions=False)
        assert m.may_walk()
