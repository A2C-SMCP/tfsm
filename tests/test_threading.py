...
from .utils import DummyModel, SomeContext, Stuff
...
class TestLockedTransitions(TestTransitions):
    ...
    #     import timeit
    #     states = ['A', 'B', 'C']
    #     tfsm = [['go', 'A', 'B'], ['go', 'B', 'C'], ['go', 'C', 'A']]
    #
    #     M1 = MachineFactory.get_predefined()
    #     M2 = MachineFactory.get_predefined(locked=True)
    #
    #     def test_m1():
    #         m1 = M1(states=states, tfsm=tfsm, initial='A')
    #         m1.get_triggers('A')
    #
    #     def test_m2():
    #         m2 = M2(states=states, tfsm=tfsm, initial='A')
    #         m2.get_triggers('A')
    #
...
class TestMultipleContexts(TestTransitions):
    def setUp(self):
        ...
        self.machine_cls = LockedMachine  # type: Type[LockedMachine]
        self.stuff = Stuff(machine_cls=self.machine_cls, extra_kwargs={"machine_context": [self.c1, self.c2]})
...
class TestLockedHierarchicalTransitions(TestNestedTransitions, TestLockedTransitions):
    ...
    def test_callbacks(self):
        ...
        class MachineModel(self.stuff.machine_cls):  # type: ignore
            def __init__(self):
                self.mock = MagicMock()
                super().__init__(self, states=["A", "B", "C"])
