...
class TestTransitions(TestCase):
    ...
    def test_init_machine_with_hella_arguments(self):
        states = [State("State1"), "State2", {"name": "State3", "on_enter": "hello_world"}]
    ...
    def test_add_states(self):
        ...
        event = s.machine.events[f"to_{s.state}"]
    ...
    def test_conditions_with_partial(self):
        ...
        s = self.stuff
        s.machine.add_transition("advance", "A", "B", conditions=partial(check, True))
        s.machine.add_transition("advance", "B", "C", unless=[partial(check, False)])
        s.machine.add_transition("advance", "C", "D", unless=[partial(check, False), partial(check, True)])
        s.advance()
        self.assertEqual(s.state, "B")
        s.advance()
        self.assertEqual(s.state, "C")
    ...
    def test_auto_transitions(self):
        ...
        # Should fail if auto tfsm is off...
    ...
    def test_ignore_invalid_triggers(self):
        ...
        b_state = State("B")
        m1 = Machine(states=[a_state, b_state], transitions=transitions, initial="B")
        with self.assertRaises(MachineError):
            m1.a_to_b()
        # Set default value on machine level
        m2 = Machine(states=[a_state, b_state], transitions=transitions, initial="B", ignore_invalid_triggers=True)
        m2.a_to_b()
        # Exception is suppressed, so this passes
        b_state = State("B", ignore_invalid_triggers=True)
        m3 = Machine(states=[a_state, b_state], transitions=transitions, initial="B")
        m3.a_to_b()
        ...
        m3 = Machine(states=[a_state, b_state], transitions=transitions, initial="B", ignore_invalid_triggers=False)
        m3.a_to_b()
    ...
    def test_state_callbacks(self):
        ...
        states = [State(name="A", on_enter="on_enter_A", on_exit="on_exit_A"), State(name="B", on_enter="on_enter_B", on_exit="on_exit_B")]
    ...
    def test_state_callable_callbacks(self):
        ...
        states = [
            State(name="A", on_enter="on_enter_A", on_exit="tests.test_core.on_exit_A"),
    ...
    def test_queued_errors(self):
        ...
        states = ["A", "B", "C"]
        transitions = [{"trigger": "do", "source": "*", "dest": "C", "before": partial(self.stuff.this_raises, ValueError)}]  # type: Sequence[TransitionConfig]

        m = Machine(states=states, transitions=transitions, queued=True, before_state_change=before_change, after_state_change=after_change)
        with self.assertRaises(MachineError):
            m.to_B(machine=m)
    ...
    def test_multiple_models(self):
        ...
        m = Machine(model=[s1, s2], states=states, initial=states[0])
        ...
        m = Machine(model=s1, states=states, initial=states[0])
    ...
    def test_dispatch(self):
        ...
        states = ["A", "B", "C"]
        m = Machine(
            model=s1, states=states, ignore_invalid_triggers=True, initial=states[0], transitions=[["go", "A", "B"], ["go", "B", "C"]]
        )
    ...
    def test_dispatch_with_error(self):
        s1, s2 = Stuff(), Stuff()
        states = ["A", "B", "C"]
        m = Machine(model=s1, states=states, ignore_invalid_triggers=True, initial=states[0], transitions=[["go", "B", "C"]])
        m.add_model(s2, initial="B")
    ...
    def test_string_trigger(self):
        ...
        self.stuff.machine.add_transition("do_raise_keyerror", "*", "C", before=partial(self.stuff.this_raises, KeyError))
    ...
    def test_get_triggers(self):
        states = ["A", "B", "C"]
        transitions = [["a2b", "A", "B"], ["a2c", "A", "C"], ["c2b", "C", "B"]]
        ...
        # self stuff machine should have to-tfsm to every state
    ...
    @skipIf(sys.version_info < (3,), "String-checking disabled on PY-2 because is different")
    def test_repr(self):
        ...
        m = Machine(
            states=["A", "B"], prepare_event=check_prepare_repr, before_state_change=check_before_repr, send_event=True, initial="A"
        )
    ...
    def test_machine_prepare(self):
        ...
        m = Machine(states=["A", "B"], transitions=transitions, prepare_event=global_callback, initial="A")
    ...
    def test_machine_finalize(self):
        ...
        transitions = [
            ...
            {"trigger": "planB", "source": "B", "dest": "A", "conditions": partial(self.stuff.this_raises, RuntimeError)},
        ]
        m = self.stuff.machine_cls(states=["A", "B"], transitions=transitions, finalize_event=finalize_mock, initial="A", send_event=True)
    ...
    def test_get_transitions(self):
        ...
        self.assertEqual({(t.source, t.dest) for t in m.get_transitions("go")}, {("A", "D"), ("B", "D"), ("C", "D")})
        self.assertEqual([(t.source, t.dest) for t in m.get_transitions(source="A", dest="D")], [("A", "D"), ("A", "D")])
        self.assertEqual(
            sorted([(t.source, t.dest) for t in m.get_transitions(dest="D")]), [("A", "D"), ("A", "D"), ("B", "D"), ("C", "D")]
        )
        self.assertEqual(
            [(t.source, t.dest) for t in m.get_transitions(source=m.states["A"], dest=m.states["D"])], [("A", "D"), ("A", "D")]
        )
        self.assertEqual(
    ...
    def test_may_transition_with_exception(self):
        ...
        stuff.machine.add_transition(
            trigger="raises", source="A", dest="B", prepare=partial(stuff.this_raises, RuntimeError("Prepare Exception"))
        )
        stuff.machine.add_transition(
            trigger="raises", source="B", dest="C", conditions=partial(stuff.this_raises, ValueError("Condition Exception"))
        )
    ...
    def test_custom_transition(self):
        ...
        a_transition = {"trigger": "go", "source": "B", "dest": "A", "my_int": 42, "my_str": "foo", "my_dict": {"bar": "baz"}}
        transitions = [["go", "A", "B"], a_transition]

        m = MyMachine(states=["A", "B"], transitions=transitions, initial="A")
        m.add_transition("reset", "*", "A", my_int=23, my_str="foo2", my_none=None, my_dict={"baz": "bar"})
        assert m.go()
